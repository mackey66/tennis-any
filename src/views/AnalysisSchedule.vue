<template>
    ...
    <div class="about">
        <h4>{{ $t("Estimated number of people") }} {{ format(today, 'MMM do (E)', {locale: locale}) }}</h4>
        <datepicker 
            v-model="picked" 
            :locale="locale"
            :weekStartsOn=0
...
<script>
    ...
    export default defineComponent({
        ...
        mounted() {
            ...
            //var sd = new Date(this.today.getFullYear(), this.today.getMonth(), this.today.getDate(), 0, 0, 0);
            //this.getAnalysisSchedule(sd);
            this.setLanguage();
        },
        methods: {
            getUser: function(date) {
                ...
                // 少し時間をおいてから取らないと1回目が取れない
                //this.getAnalysisSchedule(sd);
                this.sd = sd;
                setTimeout(this.getAnalysisSchedule, 1000);
            },
            getSchedule: function(start, end) {
                db.collection("schedule").where("config", "==", this.$store.state.config).orderBy("date").startAt(start).endAt(end)
                    .get()
                    .then((querySnapshot) => {
                        const array = [];
                        querySnapshot.forEach((doc) => {
            ...
            getScheduleTime: function(time) {
                // カウント
                var cnt = 0;
                this.ramens.forEach(ramen => {
                    var ih = ramen.date.toDate().getHours();
                ...
                const startDate = firebase.firestore.Timestamp.fromDate(sd);
                const endDate = firebase.firestore.Timestamp.fromDate(ed);
                db.collection("analysis").where("config", "==", this.$store.state.config).where("date", ">=", startDate).where("date", "<=", endDate).where("class", "==", "scheduleDaily").orderBy("date")
                    .get()
                    .then((querySnapshot) => {
                        let a = null;
                        querySnapshot.forEach((doc) => {
