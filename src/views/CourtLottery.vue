<template>
    <div class="court"><Menu class="sticky-top"></Menu>
        <div class="container-fluid">   
            <h4>{{ $t("Court Lottery") }} {{ date.year }}年{{ date.month }}</h4>
            ...
            <div class="row">
                ...
                <div v-if='$store.state.isAdmin' class="col-sm-4">          
                    ...
                    <div class="form-check-inline">
                        <input class="form-check-input" type="checkbox" value="" v-model="edit" @change="getCourtEntries()">
                        <label class="form-check-label" for="flexCheckChecked">
                            {{ $t("Edit") }}
                        </label>
                    </div> 
            ...
            <div>
                ...
                <div v-else-if='$store.state.isAdmin && edit && today >= nextMonth && entryNum > 0'>  
                    ...
                    <button class='btn btn-info btn_ btn-sm' data-toggle="modal" data-target="#setAllLotteriesDialog">{{ $t("Set All Lotteries") }}</button>
                    <button class='btn btn-secondary btn_ btn-sm' @click="clearAllLotteries()">{{ $t("Clear All Lotteries") }}</button>
                    <button class='btn btn-success btn_ btn-sm' @click="checkMember()">{{ $t("Check") }}</button>
                </div>
                <div v-else-if='$store.state.isAdmin && edit && today >= nextMonth && entryNum == 0'>
                    <button class='btn btn-primary btn_' @click="selectDateCourt()">{{ $t("Create") }}</button>
                    <button class='btn btn-secondary btn_' data-toggle="modal" data-target="#demoNormalModal2" @click="setRowIdAdd()">{{ $t("Add") }}</button>
            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class='tipscenter'>
                        <div class="form-check-inline">
                            <input class="form-check-input" type="checkbox" value="" v-model="lottery" @change="getCourtEntries()">
                            <label class="form-check-label" for="flexCheckChecked">
                                {{ $t("L:lottery") }}
                            </label>
                        </div> 
                            ...
                            <table class="table table-sm" ref="xlsxsheet" >
                                <thead >
                                    <tr>
                                        ...
                                        <th class='col court130'>{{ $t("Lottery applicant") }}</th>
                                        <th v-if='$store.state.isAdmin && $store.state.opas' class='col court60'></th>
                                ...
                                <tbody>
                                    <tr v-for="(ramen, key) in ramens" :key="key"
                                        v-bind:style="key > 0 ? (ramen.date > ramens[key-1].date ? tr1 : tr2) : tr2"
                                        >
                                        ...
                                        <td v-if='edit'>
                                            <button v-if='(ramens[key].memberName || ramens[key].lotteryMemberName) && edit' class='btn btn-warning btn-sm' @click="clearMember(ramen.id, key)">
                                                <fa icon="trash" />
                                            </button>
                                        </td>
                                        <td class="nowrap">
                                            <div v-if='edit && $store.state.isAdmin'>
                                                <draggable
                                                    class="list-group droparea2"
                                                    :list="list[key]"
                                                    :disabled="enabled[key]"
                                                    group="people"
                                                    @change="changedLotteryList"
                                                    @start="startLotteryList"
                                                    @end="endLotteryList"                   
                                                    item-key="name"
                                                    :move="checkMove_"
                                                    :data-column-id="key"
                                                >   
                                                    <template #item="{ element }">
                                                        <div>{{ element.lotteryName }}</div>
                                                    </template>
                                                </draggable>
                                            </div>
                                            <div v-else> 
                                                {{ ramens[key].lotteryMemberName }}
                                            </div>
                                        </td>
                                        <td v-if='$store.state.isAdmin && $store.state.opas'>
                                            <button class='btn btn-success btn-sm' :class='{ "hidden": !ramens[key].lotteryMemberUid }' @click="loginOpas(ramens[key].lotteryMemberUid)"><fa icon="sign-in-alt" /></button>
                                        </td>
                                        <td>
                                            <div v-if='edit'>
                    ...
                    <table class="table table-sm">
                        ...
                        >
                            <template #item="{ element }">
                                <tr>
                                    <td>
                                        <div v-if="element.check1" class="form-check">
                                            <input class="form-check-input" type="checkbox" value="" id="flexCheckChecked" v-model="element.check1" disabled>
                                        </div>
                                    </td>
                                    <td>{{ element.name }}</td>
                                    <td>{{ element.order }}</td>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">                           
                        ...
                        <button type="button" class="btn btn-primary" data-dismiss="modal" @click='downloadTypeB()'>{{ $t("Download") }}</button>
        ...
        <!-- 全抽選者セットダイアログ -->
        <div class="modal fade" id="setAllLotteriesDialog" tabindex="-1" role="dialog" aria-labelledby="modal" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="demoModalTitle"></h5>
                    ...
                    <div class="modal-body left">
                        <div>{{ $t("Set members to the vacant lotteries") }}</div>
                        <div class="form-check-inline">
                            <input class="form-check-input" type="checkbox" value="" v-model="random">
                            <label class="form-check-label" for="flexCheckChecked">
                                {{ $t("Random") }}
                            </label>
                        </div>
                    ...
                    <div class="modal-footer">                           
                        <button type="button" class="btn btn-secondary" data-dismiss="modal" >{{ $t("Cancel") }}</button>
                        <button type="button" class="btn btn-primary" data-dismiss="modal" @click='setAllLotteries()'>{{ $t("OK") }}</button> 
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                ...
                date: {
                    from: null,
                    to: null,
                    month: (new Date(this.$route.params.id)).getMonth() + 3 + "月",
        ...
        mounted() {
            ...
            this.now = new Date();
            this.today = new Date(this.today.getFullYear(), this.today.getMonth() + 3, 0)
        ...
        methods: {
                                ...
                                if (d.thursdayMeeting) {
                                    if (this.lottery) { 
                                        if (d.lottery) {
                                            ...
                                            m.kana = d.memberKana;
                                            m.docid = d.id;
                                            m.lotteryName = d.lotteryMemberName;
                                            let result = [];
                                            if (m.name || m.lotteryName) result.push(m);
                                            /*if (m.name)*/ this.list[i] = result;
                                            /*if (m.lotteryName)*/ this.list2[i] = result;
                                            i++;
                                        }                                
                                    } else {
                                        array.push(d)
            ...
            getMember: async function () {
                ...
                .then((querySnapshot) => {
                    ...
                    querySnapshot.forEach((doc) => {
                        ...
                        d.check1 = doc.data().check1
                        d.lotteryName = d.name
                        // ゲストと退会者、一時停止は除く
            ...
            clearMember: function (docid, key) {
                //this.list[key][0] = "";
                if (this.list[key]) this.list[key].splice(0, 1);
                if (this.list2[key])this.list2[key].splice(0, 1);
                this.ramens[key].memberName = "";
                this.ramens[key].memberKana = "";
                this.ramens[key].memberUid = "";
                this.ramens[key].memberNo = "";
                this.ramens[key].lotteryMemberName = "";
                this.ramens[key].lotteryMemberUid = "";
            ...
            precheckSetAllLotteries: function() {
                // modal is not a function error!
                /*
                if (this.ramens.length == 0) {
                    // モーダルを閉じる
                    $('body').removeClass('modal-open')
                    $('.modal-backdrop').remove()
                    $('#setAllLotteriesDialog').modal('hide');
                    window.alert(this.$t("No target data"))
                } else {
                    $('#setAllLotteriesDialog').modal();
                }
                */
            },
            setAllLotteries: function() {    
                //let result = window.confirm(this.$t("Set members to the vacant lotteries"))
                //if (result) {
                    let array = []
                    array = this.members.concat()
                    // シャッフル
                    if (this.random) array = this.shuffleArray(array)
                    this.ramens.forEach((item) => {
                        if (!item.lotteryMemberName && item.lottery) {
                            let name = ""
                            let id = ""
                            while(!name) {
                                const member = array.shift()
                                if (member.opasId && !member.check1) {
                                    name = member.name
                                    id = member.id
                                } 
                                if (array.length == 0) array = this.members.concat()
                            } 
                            db.collection("courtEntries").doc(item.id).update({
                                lotteryMemberName: name,
                                lotteryMemberUid: id,
                            })
                            .then(() => {
                                console.log("Document successfully written!(lotteryMemberName)");
                            })
                            .catch((error) => {
                                console.error("Error writing document: ", error);
                            });
            ...
            clearAllLotteries: function() {
                if (this.ramens.length == 0) {
                    window.alert(this.$t("No target data"))
                    return
                }
                let result = window.confirm(this.$t("Clear all lotteries?"))
                if (result) {
                    this.ramens.forEach((item) => {
                        db.collection("courtEntries").doc(item.id).update({
                            lotteryMemberName: "",
                        ...
                        .then(() => {
                            console.log("Document successfully written!(Clear lotteryMember)");
                        })
                        .catch((error) => {
                            console.error("Error writing document: ", error);
                        });                                              
            ...
            checkMember: function() {
                ...
                this.ramens.forEach((item) => {
                    if (item.lotteryMemberName) {
                        arr2.push(item.lotteryMemberName)
                    }
                });
                const arr3 = arr1.filter(i => arr2.indexOf(i) == -1)
                if (arr3.length > 0) {
                    console.log(arr3)
                    window.alert(this.$t("The following persons have not been entered into the lottery") + "\n" + arr3)
                } else {
                    window.alert(this.$t("Everyone is in the lottery"))
                }
            ...
            updateCourtLotteryMember: function(docid, id, name) {
                    db.collection("courtEntries").doc(docid).update({
                        lotteryMemberUid: id,
                        lotteryMemberName: name   
                    })
            ...
            endDistList: function(evt) {
                ...
                // ?
                //this.ramens[toIndex].lotteryMemberName = src.memberName;
                // DB更新
                this.updateCourtEntryMember(dst.id, src.memberUid, src.memberName, src.memberKana, src.memberNo);
                this.updateCourtLotteryMember(dst.id, src.memberUid, src.memberName);
                // 元をクリア
                this.clearMember(src.id, fromIndex);
            },
            startLotteryList: function(evt) {
                console.log("start evt", evt)
                this.dragstart = "lottery"
            },
            endLotteryList: function(evt) {
                // 
                console.log("endLotteryList(): " + evt)
                const from = evt.from.outerHTML;
                const fromId = from.match( /data-column-id="[0-9]+"/ )[0];
                const fromIndex = fromId.match(/"(.*?)"/)[1];
                const to = evt.to.outerHTML;
            ...
            endSourceList: function(evt) {
                console.log("endSourceList")
                //console.log(evt)
                //console.log(evt.to.outerHTML) 
                // メンバーリストからドラッグした場合               
                ...
                // DB更新
                if (cname[0].match( /droparea2/ )) {
                    this.updateCourtLotteryMember(dst.id, src.id, src.name);
                    console.log(src)
                    //dst.name = ""
                } else {    
                    this.updateCourtEntryMember(dst.id, src.id, src.name, src.kana, src.no);
                }
                // Draggableの同じGroupに意図しないデータが入るので読み込み直す
                this.getCourtEntries()                        
            },
            changedDistList: function(evt) {
                //console.log("changedDistList")
                console.log(evt);               
                if (evt.added) {
                    // 追加の場合
            ...
            changedLotteryList: function(evt) {
                //console.log("changedLotteryList")
                console.log(evt);               
                if (evt.added) {
                    // 追加の場合
                    console.log("changedLotteryList(added)")
                    if (evt.added.newIndex > 1) {
                        console.log("over 1!")
                        return false;
                    }
                    ...
                    // 移動の場合
                    console.log("changedLotteryList(moved)")
                }              
            },          
            clearList: function() {
            ...
            checkMove: function(evt) {
                this.dragTargetLength = 0
                console.log("checkMove()")
                console.log(evt)
                console.log("list: " + evt.relatedContext.list)
                console.log(evt.to.outerText)
                if (evt.to.outerText) {
                    console.log("drug cancel (outerText)")
                    return false
                }
                if (evt.relatedContext.list) {
                    this.dragTargetLength = evt.relatedContext.list.length
                } else {
                    //console.log(evt)
                    //this.dragTargetLength = evt.relatedContext.list2.length
                }
                /*  
                if (this.dragTargetLength > 0) {
                    // ターゲットが1件以上あるときはドラッグをキャンセル
                    console.log("drug cancel (list.length)")
                    return false
                }
                */
                return true;
            ...
            */
            ...
            downloadTypeA: async function() {
                ...
                worksheet["!cols"] = [ { wch: 10 }, { wch: 12 }, { wch: 4 }, { wch: 9 }, { wch: 11 }, { wch: 11 }, { wch: 15 } ];
                // Style
                /*
                ...
                */
                ...
                XLSX.writeFile(workbook, this.$t("Lottery") + this.date.year + ('0' + this.date.month.replace(/[^0-9]/g, '')).slice(-2) + '_' + this.formatDate(new Date(), 'yyMMdd') + '.xlsx');
            },
            downloadTypeB: async function() {
                ...
                XLSX.writeFile(workbook, this.$t("Lottery") + this.date.year + ('0' + this.date.month.replace(/[^0-9]/g, '')).slice(-2) + '.xlsx');
            },
